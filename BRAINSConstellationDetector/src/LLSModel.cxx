...
::Read()
{
  if( this->m_FileName == "builtinDefaultModel" )
    {
    std::cout << "Using internal default model." << std::endl;
    this->SetDefaultModel(); //Read default model from hard-coded values
    return 0;
    }
  try
    {
...
#ifndef FORCE_WRITE_CXX_INITIALIZATION
# define FORCE_WRITE_CXX_INITIALIZATION 0
#endif
#if FORCE_WRITE_CXX_INITIALIZATION > 0
  /* This code is almost always commented out,  It is only here to auto-generate
   * the default initializaitons needed so that we do not need input files all the
   * time.
  ...
  */
  ...
  std::ofstream lsout;
  const std::string ModelCodeFileName(
    "/Users/johnsonhj/src/BRAINSTools/BRAINSConstellationDetector/src/LLSModel_defaults.cpp");
  lsout.open(ModelCodeFileName);

  lsout << "#include \"LLSModel.h\"" << std::endl;
  lsout << "void LLSModel::SetDefaultModel()" << std::endl;
  lsout << "{" << std::endl;
  itk::NumberToString<double>    doubleToString;
  lsout << "//This file autogenerated from " __FILE__ << "  : " << __LINE__ << std::endl;
  lsout << "//during a manually identified compile time flag.";
  lsout << "/* Write C++ representation of of m_LLSMeans */" << std::endl;
  lsout << "m_LLSMeans.clear();" << std::endl;
  for(LLSMeansType::const_iterator it= m_LLSMeans.begin(); it != m_LLSMeans.end(); ++it ){
    const std::string nm=std::string("tempArr")+it->first;
    lsout << "static const double "<< nm <<"[] = {";
    for(size_t i = 0; i < it->second.size() ; ++i)
    {
      lsout << doubleToString(it->second[i]);
    ...
    lsout << "m_LLSMeans[\"" << it->first << "\"] = "
              <<"std::vector<double>(" << nm << "," << nm
              <<"+sizeof(" << nm << ")/sizeof(" << nm << "[0]));"
              << std::endl;
  }
  lsout << "/* Write C++ representation of of m_LLSMatrices */" << std::endl;
  lsout << "m_LLSMatrices.clear();" << std::endl;
  for(LLSMatricesType::const_iterator it= m_LLSMatrices.begin(); it != m_LLSMatrices.end(); ++it ){
    const std::string nm=std::string("matrixDataBlock")+it->first;
    lsout << "static const double "<< nm <<"["
          << it->second.rows()*it->second.cols()<< "] = {";
    for(size_t c=0; c< it->second.cols(); ++c) {
        for(size_t r=0; r< it->second.rows(); ++r) {
          lsout << doubleToString(it->second(r, c));
          if ( (c*r+r) < (it->second.cols()*it->second.rows()-1)) {
            lsout << ",";
          }
        }
    }
    lsout << "};" << std::endl;
    lsout << "m_LLSMatrices[\"" << it->first << "\"] = "
              << "vnl_matrix<double>(" << nm << ","
              << it->second.rows() << "," << it->second.cols() << ");" << std::endl;
  }
  lsout << "/* Write C++ representation of of m_LLSSearchRadii */" << std::endl;
  lsout << "m_LLSSearchRadii.clear();" << std::endl;
  for(LLSSearchRadiiType::const_iterator it= m_LLSSearchRadii.begin(); it != m_LLSSearchRadii.end(); ++it ){
    lsout << "m_LLSSearchRadii[\"" << it->first << "\"] = " << doubleToString(it->second ) << ";" << std::endl;
  }
  lsout << "}" << std::endl;
  lsout.close();

  std::cerr << __FILE__ << "  : " << __LINE__ << std::endl;
  itkGenericExceptionMacro(<< "ERROR BEING WRITTEN:  DO NOT CONTINUE WITH THIS CODE COMMENTED IN.\n"
    << ModelCodeFileName);
#endif
  return 0;
}
